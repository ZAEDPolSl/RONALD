...
SUPPORTED_EXTENSIONS = [".nrrd", ".nii.gz", ".nifti", '.dcm', '.mhd']


class ImageInstance:
    def __init__(self, show_exceptions=True, series_uid_prio='most_slices', anonymize=False, anon_patient_id=None,
                 func=None):
        ...
        if os.path.isdir(input_path):
            ...
            if series_uid is None:
                if self.series_uid_prio == 'first':
                    ...
                    raise ValueError(f"The {self.series_uid_prio} is not proper name for he priority mode,"
                                     f" choose either 'most_slices', 'condition' or 'first'!")
            if series_uid is None:
                print(f"Could not read the image! Check if under path {input_path} are stored DICOM images or change "
            ...
            # get series file names
                ...
                series_file_names = sitk.ImageSeriesReader.GetGDCMSeriesFileNames(os.path.join(input_path, series_uid), series_uid)
            ...
            if self._validate_dicom_series(series_file_names):
                ...
                self.root_folder = os.path.split(series_file_names[0])[0]  # remember root folder
            else:
                if self.show_exceptions:
                    raise TypeError(f"Folder {os.path.basename(input_path)} does not contain a valid DICOM series!")
                else:
                    return False
    ...
    def read(self, input_path, series_uid=None):
        ...
        if self.image_reader is None:
            ...
            print("ImageInstance already occupied, create new ImageInstance if you want to load new image!")
            input_image = self.image_reader.Execute()
        return input_image
    ...
    def generate_patient_anon_id(self):
        if self.patient_id is None:
            ...
            if self.anon_patient_id is None:
                self.anon_patient_id = modification_date + modification_time + str(random.randint(100000, 999999))
        else:
    ...
    def write(self, image, output_path, description='Segmentation', relate_dirs_to_uids=False, forced_mode=None,
              series_number=None, force_study_uid_change=None):
        """
        Write DICOM or nrrd.
    ...
    def _write_dicom(self, image: sitk.Image, output_path, description='Segmentation', relate_dirs_to_uids=False,
        ...
        if self.image_reader is not None and not self.anonymize:
            ...
            if self.new_study_uid is None:
                self.new_study_uid = "1.2.826.0.1.3680043.10.877." + modification_date \
                                     + ".2" + modification_time + ".2" \
        ...
        if force_study_uid_change is not None:
            if self.new_study_uid is None:
                self.new_study_uid = "1.2.826.0.1.3680043.10.877." + modification_date + ".2"\
                                     + modification_time + ".2" \
                                     + str(random.randint(100000, 999999))
        ...
        if type(self.image_reader) is sitk.ImageFileReader:
            series_tag_values = [(k, self.image_reader.GetMetaData(0, k)) for k in tags_to_copy if
                                 self.image_reader.HasMetaDataKey(k)] + \
        ...
        for instance_number, i in zip(range(image.GetDepth()), range(image.GetDepth() - 1, -1, -1)):
            ...
            if self.new_study_uid is not None:
                image_slice.SetMetaData("0020|000d", str(self.new_study_uid))  # Study Instance UID
    ...
    def get_series_by_condition(input_path, func):
        ...
        for nested_dir in listdirs(input_path):
            ...
            for series_id in series_ids:
                file_names = sitk.ImageSeriesReader.GetGDCMSeriesFileNames(nested_dir, series_id)
